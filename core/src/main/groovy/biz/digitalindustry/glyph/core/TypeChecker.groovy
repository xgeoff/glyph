...
class TypeChecker {
    ...
    private void checkFunction(FunctionDecl fn) {
        ...
        fn.body.statements.each { stmt ->
            switch (stmt) {
                case VarDecl:
                    handleVarDecl(stmt, env, mutability); break
                case AssignStmt:
                    handleAssign(stmt, env, mutability); break
                case PrintStmt:
    ...
    private void handleVarDecl(VarDecl stmt, Map<String, TypeRef> env, Map<String, Mutability> mutability) {
    ...
    private void handleAssign(AssignStmt stmt, Map<String, TypeRef> env, Map<String, Mutability> mutability) {
        ...
        switch (expr) {
            ...
            case LambdaExpr:
                return new LambdaType(
                        expr.params.collect { new ParameterType(it.name, resolveType(it.type, it.pos)) },
                        resolveType(expr.returnType, expr.pos)
                )
            default:
    ...
    private TypeRef inferMatch(MatchExpr expr, Map<String, TypeRef> env) {
        ...
        expr.cases.each { c ->
            TypeRef keyType = inferExpr(c.key, env)
            if (!isAssignable(keyType, target) || !isAssignable(target, keyType)) {
                throw new IllegalArgumentException("match key type ${format(keyType)} does not match target ${format(target)}${pos(c.pos)}")
            }
            TypeRef caseType = inferExpr(c.value, env)
            if (valueType == null) {
                valueType = caseType
            } else if (valueType != caseType) {
                throw new IllegalArgumentException("match case types differ: ${format(valueType)} vs ${format(caseType)}${pos(c.pos)}")
            }
        }
        if (expr.elseExpr == null) {
            throw new IllegalArgumentException("match requires else clause${pos(expr.pos)}")
        }
        TypeRef elseType = inferExpr(expr.elseExpr, env)
            if (valueType != null && (!isAssignable(valueType, elseType) || !isAssignable(elseType, valueType))) {
                throw new IllegalArgumentException("match else type ${format(elseType)} does not match cases ${format(valueType)}${pos(expr.pos)}")
            }
        return valueType ?: elseType
    }
    ...
    private TypeRef resolveType(String name, SourcePos p) {
        ...
        if (name.endsWith('?')) {
            ...
            return new NullableType(resolveType(inner, p))
        }
        if (name.startsWith('[') && name.endsWith(']')) {
            ...
            if (inner.contains(':')) {
                ...
                return new MapType(resolveType(key, p), resolveType(value, p))
            }
            return new ArrayType(resolveType(inner, p))
        }
        switch (name) {
